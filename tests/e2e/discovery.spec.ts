...
test('robots and sitemap preserve governed public discovery', async ({ request }) => {
  ...
  for (const { routePath, defaultPath } of [
    ...
    { routePath: '/schools/psychology', defaultPath: '/en/schools/psychology' },
  ]) {
    for (const locale of locales) {
      const href = `${sitemapUrl.origin}/${locale}${routePath}`;
      expect(sitemap).toMatch(
        new RegExp(
          `hreflang=["']${locale}["'][^>]+href=["']${escapeRegExp(href)}["']|href=["']${escapeRegExp(href)}["'][^>]+hreflang=["']${locale}["']`,
        ),
      );
    }
    ...
    expect(sitemap).toMatch(
      new RegExp(
        `hreflang=["']x-default["'][^>]+href=["']${escapeRegExp(defaultHref)}["']|href=["']${escapeRegExp(defaultHref)}["'][^>]+hreflang=["']x-default["']`,
      ),
    );
  }
});
