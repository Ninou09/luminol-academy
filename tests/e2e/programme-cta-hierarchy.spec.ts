...
for (const locale of ['en', 'fr', 'ar'] as const) {
  test(`${locale} programme cards and details keep one clear programme-specific conversion path`, async ({
    page,
  }) => {
    ...
    await expect(links.nth(2)).toHaveAttribute(
      'href',
      new RegExp(`^/${locale}/schools/(psychology|languages|training)#programs$`),
    );
    ...
    const detailPrimaryActions = page.locator(
      '[data-programme-detail-primary-action]',
    );
    await expect(detailPrimaryActions).toHaveCount(2);

    const firstDetailHref = await detailPrimaryActions
      .first()
      .getAttribute('href');
    expect(firstDetailHref).toMatch(
      new RegExp(`^/${locale}/contact\\?programme=[^&#]+$`),
    );
    await expect(detailPrimaryActions.last()).toHaveAttribute(
      'href',
      firstDetailHref!,
    );

    const primaryLabel = (
      (await detailPrimaryActions.first().textContent()) ?? ''
    ).trim();
    expect(primaryLabel).not.toBe('');
    await expect(detailPrimaryActions.last()).toHaveText(primaryLabel);
  });
}
