...
test('localized home pages render the governed organization and website structured data', async ({
  page,
}) => {
  for (const locale of ['en', 'fr', 'ar'] as const) {
    ...
    const websiteJsonLd = JSON.parse(rawWebsiteJsonLd!) as Record<string, unknown>;
...
test('localized About pages expose the verified founder identity without invented claims', async ({
  page,
}) => {
  for (const locale of ['en', 'fr', 'ar'] as const) {
    ...
    const founderJsonLd = JSON.parse(rawFounderJsonLd!) as Record<string, unknown>;
    const canonicalHref = await page
      .locator('link[rel="canonical"]')
      .getAttribute('href');
    expect(canonicalHref).toBeTruthy();
