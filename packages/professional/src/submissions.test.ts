...
describe('professional project submissions', () => {
  it('accepts a valid learner project submission', () => {
    expect(
      projectSubmissionSchema.parse({
        ...
        reflection: 'This project demonstrates how I applied the course competencies.',
      }),
  ...
  it('allows a revised submission to be submitted again', () => {
    expect(() => assertSubmissionTransition('REVISION_REQUIRED', 'SUBMITTED')).not.toThrow();
  });
  ...
  it('requires revision even when the numeric score is passing', () => {
    expect(
      determineReviewOutcome({
        ...
        score: 82,
        feedback: 'Strong work, but the evidence section needs a clearer explanation.',
  ...
  it('approves submissions at the passing boundary', () => {
    expect(
      determineReviewOutcome({
        reviewerId: 'reviewer-1',
        score: 70,
        feedback: 'The project meets the required competency evidence standard.',
  ...
  it('rejects submissions below the passing boundary', () => {
    expect(
      determineReviewOutcome({
        reviewerId: 'reviewer-1',
        score: 69,
        feedback: 'The submission does not yet demonstrate the required competencies.',
        requiresRevision: false,
      }),
