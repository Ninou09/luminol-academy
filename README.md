...
Clerk owns sign-in identity and signed webhooks synchronize users into PostgreSQL. Configure a Clerk webhook for `/api/webhooks/clerk` with `user.created`, `user.updated`, and `user.deleted` events. RBAC roles and permissions are stored in Prisma; protected server operations must use the helpers from `@luminol/auth/server` and `@luminol/auth/authorization`. Run `pnpm --filter @luminol/database seed` for development-only roles, users, permissions, and sample academy data.

Sanity Studio uses the corresponding `SANITY_STUDIO_PROJECT_ID` and `SANITY_STUDIO_DATASET` variables. Email and analytics adapters remain available through shared packages for later product milestones.
